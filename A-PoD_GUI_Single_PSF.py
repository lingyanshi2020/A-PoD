...
def run():
    ...
    #bgimage = spline_filter(image, order=3)

    image = posimage(image-bgimage-bground)
    ...
    while (iternumb - jj)/iternumb > 0.01:
        ...
        if gpus:
          ...
          try:
            tf.config.experimental.set_virtual_device_configuration(gpus[0], [tf.config.experimental.VirtualDeviceConfiguration(memory_limit=0.9*G_memory)])
