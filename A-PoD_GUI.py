...
def run():
    ...
    def posimage(image):
        ...
        return image
    def posimage_tf(image):

        # Use TensorFlow's greater_equal and where functions to set negative values to zero
    ...
    def gradgenfun(re_image, imsize, temp_addr_x, temp_addr_y, psf, sizeaddr, imagefilter):
        ...
        grad_step = tf.constant(4, dtype=tf.int32)
    ...
    for iii in range(0, np.shape(image)[0]):
        for jjj in range(0, np.shape(image)[1]):
            dist_image[iii][jjj] = np.sqrt(np.square(iii - np.shape(image)[0]/2) + np.square(jjj - np.shape(image)[1]/2))
    ...
    if np.min(image)>0:
        ...
        thrval = np.min(image) + 0.2*np.std(lp_filt_2(image, big_psf, image.shape))

    else:

        thrval = 0 + 0.2*np.std(lp_filt_2(image, big_psf, image.shape))
    ...
    io.imsave(fileaddr + filename+'_lp_filter.tif', bgimage)
    ...
    #real_psf = 1.0*psf_ratio*px_ratio*SNR_ratio*big_psf*intratio
    #big_psf = 1.0*psf_ratio*px_ratio*SNR_ratio*big_psf*intratio
    ...
    while (iternumb - jj)/iternumb > 0.01:
        ...
        if gpus:
          ...
          try:
            tf.config.experimental.set_virtual_device_configuration(gpus[0], [tf.config.experimental.VirtualDeviceConfiguration(memory_limit=G_memory)])
    ...
    #f_rimage = f_rimage/np.sum(f_rimage)*sum_val_image
    ##f_rimage = f_rimage + bgimage
